        ...
        <div class="relative w-5/6 h-screen bg-gray-100">
            ...
            <div class="m-16 px-8 py-2 bg-white">
                <div class="flex flex-row justify-between items-center py-4">
                    <h3 class="text-xl font-semibold">Edit Data Mahasiswa</h3>
                </div>
                <?php foreach ($mhs as $mhs): ?>
                <form action="<?= base_url().'index.php/mhs/update' ?>" method="post"
                    class="w-full flex items-center flex-col">
                    <div class="w-2/5 flex flex-col mb-6">
                        <h5>Nama</h5>
                        <input type="hidden" name="id" value="<?= $mhs->id ?>">
                        <input type="text" name="nama" placeholder="Nama" value="<?= $mhs->nama ?>"
                    ...
                    <div class="w-2/5 flex flex-col mb-6">
                        <h5>Semester</h5>
                        <input type="text" name="semester" placeholder="Semester" value="<?= $mhs->semester ?>"
                            class="border-2 border-gray p-2 rounded focus:outline-none focus:ring-1 focus:border-blue-300">
                    </div>
                    <div class="w-2/5 flex flex-col mb-6">
                        <h5>Email</h5>
                        <input type="email" name="email" placeholder="Email" value="<?= $mhs->email ?>"
                            class="border-2 border-gray p-2 rounded focus:outline-none focus:ring-1 focus:border-blue-300">
                    </div>
                    <button type="submit"
