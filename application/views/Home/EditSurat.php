        ...
        <div class="relative w-5/6 h-screen bg-gray-100">
            ...
            <div class="m-16 px-8 py-2 bg-white">
                <div class="flex flex-row justify-between items-center py-4">
                    <h3 class="text-xl font-semibold">Edit Surat</h3>
                </div>
                <?php foreach ($surat as $srt): ?>
                <form action="<?= base_url().'index.php/surat/update' ?>" method="post"
                    class="w-full flex items-center flex-col">
                    <div class="w-2/5 flex flex-col mb-6">
                        <h5>Orang Tua</h5>
                        <input type="hidden" name='nomor' value="<?= $srt->Nomor ?>">
                        <input type="text" name="ortu" placeholder="Orang Tua" value="<?= $srt->ortu ?>"
                            class="border-2 border-gray p-2 rounded focus:outline-none focus:ring-1 focus:border-blue-300">
                    </div>
                    <div class="w-2/5 flex flex-col mb-6">
                        <h5>NIP/NRP/NOPEN</h5>
                        <input type="text" name="nip" placeholder="NIP/NRP/NOPEN" value="<?= $srt->nip ?>"
                            class="border-2 border-gray p-2 rounded focus:outline-none focus:ring-1 focus:border-blue-300">
                    </div>
                    <div class="w-2/5 flex flex-col mb-6">
                    ...
                    <div class="w-2/5 flex flex-col mb-6">
                        <h5>Instansi</h5>
                        <input type="text" name="instansi" placeholder="Instansi" value="<?= $srt->instansi ?>"
                            class="border-2 border-gray p-2 rounded focus:outline-none focus:ring-1 focus:border-blue-300">
                    </div>
                    <div class="w-2/5 flex flex-col mb-6">
                        <h5>Kepentingan</h5>
                        <input type="text" name="kepentingan" placeholder="Kepentingan" value="<?= $srt->Kepentingan ?>"
                            class="border-2 border-gray p-2 rounded focus:outline-none focus:ring-1 focus:border-blue-300">
                    </div>
                    <button type="submit"
                        class="p-2 bg-blue-600 mb-6 rounded text-white font-semibold hover:bg-blue-700">Edit</button>
                </form>
                <?php endforeach ?>
            </div>
        </div>
